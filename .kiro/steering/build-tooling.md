...
- Configuration: `tsdown.config.ts` at root (workspace mode)
...
- Workspaces: `packages/*` in root `package.json`

## Release

### bonvoy
- **bonvoy** for releases (NOT semantic-release, changesets)
- Independent versioning (each package versioned separately)
- `npx bonvoy shipit` in CI
